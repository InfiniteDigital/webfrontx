...
function callXMLHttpRequest(url, data, token) {
    let result;
...
function numberReward(textNumber) {
    if (parseFloat(textNumber) > 0) {
        return `<span class="text-danger">${textNumber}</span>`
    } else {
        return `<span class="text-success">${textNumber}</span>`
    }
}

function numberColor(textNumber) {
...
function handleApiFaild(resData) {
    if (resData.statusCodeText == "401") {
        Swal.fire({
            title: "แจ้งเตือน",
            ...
            if (result.value) {
                ...
                window.location.href = "../../standard_utility/login.html";
            }
        });
    }
}

function convertTHDateTime(dateTh) {
    const dataArr = dateTh.split(' ')
    const dateThArr = dataArr[0].split('/')
    const dateEn = `${dateThArr[2]}-${dateThArr[1]}-${dateThArr[0]} ${dataArr[1]}:00`
    return dateEn
}

function genObjectSerializeArray(formData, attHidArray = []) {
    let data = {};
    formData.forEach((item, key) => {
        if (data[item.name] === undefined) {
            let findHidArr = attHidArray.find((item2, key2) => {
...
function genObjectSerializeArray2(formData, attHidArray = []) {
    let data = {};
    formData.forEach((item, key) => {
        if (data[item.name] === undefined) {
            let findHidArr = attHidArray.find((item2, key2) => {

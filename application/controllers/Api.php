...
class Api extends CI_Controller
{
	...
	public function alert_topup($t = 1687515250, $trr = null)
	{
		// echo json_encode(['status' => false]);
		$user_session = $this->session->userdata('user');
		if ($user_session) {
	...
	function checkCapcha($token = null)
	{
		...
		curl_setopt_array($curl, array(
			...
			CURLOPT_POSTFIELDS     => "secret=" . $setting['re_secretkey'] . "&response=" . $token,
	...
	public function check_bank()
	{
		...
		if (empty($token)) {
			exit(json_encode(['status' => 500, 'msg' => 'ใช้งานไม่ถูกต้อง']));
		}
		$check = $this->checkCapcha($token);
		if (!$check) {
			exit(json_encode(['status' => 501, 'msg' => 'ใช้งานไม่ถูกต้อง']));
		}
		$data = [
			"bank_name" => $bank,
			"bank_number" => $accnum,
		...
		$curl = curl_init();
		curl_setopt_array($curl, array(
			CURLOPT_URL            => $seting['api_internal'] . '/checkBank',
			CURLOPT_RETURNTRANSFER => true,
			CURLOPT_TIMEOUT        => 0,
			CURLOPT_CUSTOMREQUEST  => "POST",
		...
		if ($res->statusCode == 200) {
			echo json_encode(['status' => 200, 'name' => $res->data->accountToName]);
		} else {
			echo json_encode(['status' => 500, 'msg' => 'ไม่พบเลขบัญชีของคุณ']);
		}
	}

	public function check_bank_old()
	{
		$bank   = $this->input->post('bank_name');
		$accnum = $this->input->post('bank_number');
		$banks  = array(
			...
			"ttb" => array("name" => "ทหารไทย", "code" => "011"),
	...
	public function register()
	{
		if ($_POST) {
			...
			$username = $this->input->post('username');
			$check    = $this->user_model->auth($username);
			$check1   = $this->user_model->chekaccnumrow($this->input->post('bank_number'));
			...
			if ($check > 0) {
			...
			} elseif ($this->input->post('fname') == '' || strlen($this->input->post('fname')) < 5) {
				echo json_encode(['status' => false, 'msg' => 'กรุณากรอกชื่อบัญชีให้ครบถ้วน']);
				// } elseif ($check3 > 0) {
				//     echo json_encode(['status' => false, 'msg' => 'มีเบอร์นี้แล้ว']);
				...
				if ($res['success'] == true) {
					...
					// 
					// $res_user_ufa = $this->ufa->AddMember($this->input->post('username'));
					// $new_user_ufa = $res_user_ufa->username;
					// $new_pass_ufa = $res_user_ufa->newpass;
					...
					$new_pass_ufa = $pass;
					// exit(json_encode(['status' => false, 'msg' => $new_pass_ufa]));die();
	...
	private $keyJwt2 = 'uoHCFT7VeYXzVjqqtNc4';

	public function genAuthJwt($memberId = false)
	{
		$user_session = $this->session->userdata('user');
		if ($user_session) {
			$payload = [
			...
			$result = JWT::encode($payload, $this->keyJwt, 'HS256');
			echo json_encode(['stats' => true, 'token' => $result]);
		}
	}
	public function genAuthJwtUser($memberId = false)
	{
		$user_session = $this->session->userdata('user');		
		if ($user_session) {
			$payload = [
			...
			$result = JWT::encode($payload, $this->keyJwt2, 'HS256');
			echo json_encode(['stats' => true, 'token' => $result]);
		}
	}
}

...
class Ufa
{
	...
	private $urlmain = "https://ocean.apple855.com";
	private $urllogin = "https://ocean.apple855.com/Public/Default11.aspx";
	private $urlagent_info = "https://ocean.apple855.com/AccInfo.aspx";
	private $urlbalance = "https://ocean.apple855.com/_SubAg/AccBal.aspx?role=sa";
	private $urlmem_info = "https://ocean.apple855.com/_SubAg1/MemberSet.aspx?&userName=";
	private $urlturnover = "https://ocean.apple855.com/_SubAg/SubAccsWinLoseFull.aspx?role=sa&userName=";
	private $urlmember = "https://ocean.apple855.com/_SubAg1/MemberSet.aspx";
	...
	public function __construct()
	{
		...
		$this->sub = $setting['sub_agent'];
		// 		print_r($this->user_agent);die;
		$this->setCookieFile(APPPATH . $this->user_agent . '.ops');
		$this->checkSub($this->sub);
		$this->check_login();
	}
	...
	private function checkSub($ss = null)
	{
		...
		if (!$this->sub) {
			$this->urlmain = "https://ocean.apple855.com";
			$this->urllogin = "https://ocean.apple855.com/Public/Default11.aspx";
			$this->urlagent_info = "https://ocean.apple855.com/AccInfo.aspx";
			$this->urlbalance = "https://ocean.apple855.com/_Age/AccBal.aspx?role=ag";
			$this->urlmem_info = "https://ocean.apple855.com/_Age1/MemberSet.aspx?&userName=";
			$this->urlturnover = "https://ocean.apple855.com/_Age/SubAccsWinLoseFull.aspx?role=ag&userName=";
			$this->urlmember = "https://ocean.apple855.com/_Age1/MemberSet.aspx";
		}
	}
	...
	public function check_login()
	{
		...
		$check = ($this->sub) ? "//span[@id='lblsCashBalance']" : "//span[@id='lblaCashBalance']";
		$res = $this->DOMXPath($this->response, $check);
	...
	public function agent_info()
    {
		...
		$this->headers[] = "x-api-key: f5dac0a9-ed80-4e43-a254-cab700154cac";
        $data = array(
            "agentUsername" => $this->user_agent,
            "agentPassword" => $this->pass_agent,
        );
        $this->request('https://kraken.mrwed.cloud/partner/agent/info', '', json_encode($data));

        $lbds = json_decode($this->response);
		$total = $this->CI->admin_model->total_userufa();
        if ($lbds->status == "success") {
            $data = [
                "lblaUserName" => $lbds->data->agentUsername,
                "lblaCredit" => $lbds->data->remainingCredit,
                "lblaCashBalance" => null,
                "lblaAccBalance" => null,
                "lblaTotalBalance" => null,
                "lblaTotalOutstanding" => null,
                "lblaTotalAgentCredit" => null,
                "lblaTotalMemberCredit" => null,
                "lblaCurrency" => null,
                "lblaTotalGivenCredit" => $lbds->data->creditLimit,
                "lblaMinBet" => $lbds->data->minBet . "/" . $lbds->data->maxBet,
                "lblaTotalAgentCount" => null,
                "lblaTotalMemberCount" => number_format($total,0),
            ];
        } else {
            $data = [
                "lblaUserName" => null,
                ...
                "lblaTotalMemberCredit" => null,
                "lblaCurrency" => null,
                "lblaTotalGivenCredit" => null,
                "lblaMinBet" => null,
                "lblaTotalAgentCount" => null,
                "lblaTotalMemberCount" => null,
            ];
        }
	...
	public function agent_info22()
	{
		if ($this->is_login) {
			...
			if($this->sub){
				...
				$lblaUserName = $this->DOMXPath($this->response, "//span[@id='lblaUserName']");
				$lblaCashBalance = $this->DOMXPath($this->response, "//span[@id='lblaCashBalance']");
				$lblaAccBalance = $this->DOMXPath($this->response, "//span[@id='lblaAccBalance']");
				$lblaTotalBalance = $this->DOMXPath($this->response, "//span[@id='lblaTotalBalance']");
				$lblaTotalOutstanding = $this->DOMXPath($this->response, "//span[@id='lblaTotalOutstanding']");
				$lblaTotalAgentCredit = $this->DOMXPath($this->response, "//span[@id='lblaTotalAgentCredit']");
				$lblaTotalMemberCredit = $this->DOMXPath($this->response, "//span[@id='lblaTotalMemberCredit']");
				$lblaCurrency = $this->DOMXPath($this->response, "//span[@id='lblaCurrency']");
				$lblaTotalGivenCredit = $this->DOMXPath($this->response, "//span[@id='lblaTotalGivenCredit']");
				$lblaMinBet = $this->DOMXPath($this->response, "//span[@id='lblaMinBet']");
				$lblaTotalAgentCount = $this->DOMXPath($this->response, "//span[@id='lblaTotalAgentCount']");
				$lblaTotalMemberCount = $this->DOMXPath($this->response, "//span[@id='lblaTotalMemberCount']");
				$lblaCredit = $this->DOMXPath($this->response, "//span[@id='lblaCredit']");
			}
	...
	public function Playgame($username = null, $password = null)
    {
        if (!empty($username) && !empty($password)) {
            ...
            $this->request('https://kraken.mrwed.cloud/partner/auth/login', '', json_encode($data));
		...
		if (!empty($username) && !empty($amount)) {
			if ($this->is_login) {
				...
				if ($lblStatus[0]->nodeValue == '') {
					...
					return ['error' => $lblStatus[0]->nodeValue, "status" => false];
				}
			}
		}
	}
		...
		if (!empty($username) && !empty($start) && !empty($end)) {
			if ($this->is_login) {
				...
				for ($i = 2; $i <= count($rows) - 2; $i++) {
					...
					$winloss = $winloss + ((floatval($s[9]) * -1) + floatval($s[10]));
					$user_data[$user] = [
						"username" => $user,
						"turnover" => floatval(preg_replace('/\.(?=.*\.)/', '', str_replace(",", ".", $s[3]))),
						"winloss" => (floatval($s[9]) * -1) + floatval($s[10])
					];
				}
	...
	public function TurnOverAuto($username = null, $start = null, $end = null)
	{
		if (!empty($start) && !empty($end)) {
			if ($this->is_login) {
				$this->headers = $this->getHeaders();
				...
				for ($i = 2; $i <= count($rows) - 2; $i++) {
					...
					$s =  explode("/", $data);
					// floatval(preg_replace('/\.(?=.*\.)/', '', str_replace(",", ".", $s[3])));
					// floatval(preg_replace('/\.(?=.*\.)/', '', str_replace(",", ".", $s[10])));
					$user = str_replace("*", "", $s[0]);
					// $winloss = $winloss + ((floatval($s[9]) * -1) + floatval($s[10]));
					$user_data[$user] = [
						"username" => $user,
						"turnover" => floatval(preg_replace('/\.(?=.*\.)/', '', str_replace(",", ".", $s[3]))),
						"winloss" => floatval(preg_replace('/\.(?=.*\.)/', '', str_replace(",", ".", $s[10])))
					];
				}
				return 	$user_data;
			}
			...
			if ($lblatatus[0]->nodeValue == "Profile updated successfully.") {
				...
				if ($res != false) {
					if ($res->msg->nodeValue == "รหัสผ่านเปลี่ยนแปลงประสบความสำเร็จ.") {
						return (object)['username' =>  strtolower($res->user), "newpass" => $res->newpass];
					} else {
	...
	{
		$this->setCookieFile(FCPATH, "betufa");
		$this->headers = $this->getHeaders();
	...
	private function getHeaders()
	{
		$header = array(
			"cache-control: no-cache",
			"origin: https://ocean.apple855.com",
			"user-agent: Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/80.0.3987.163 Safari/537.36"
		);
		return $header;
	}
	...
	public function WinLossUser($username = null, $start = null, $end = null)
	{
		if (!empty($username) && !empty($start) && !empty($end)) {
			if ($this->is_login) {
				$this->headers = $this->getHeaders();
				...
				for ($i = 2; $i <= count($rows) - 2; $i++) {
					$data = preg_replace("/\s+/", "/", trim($rows[$i]->nodeValue));
					$s =  explode("/", $data);
					$user = strtolower(str_replace("*", "", $s[0]));
					$username = strtolower($username);
					// $winloss = $winloss + ((floatval($s[9]) * -1) + floatval($s[10]));
					$user_data[$user] = [
						"username" => $user,
	...
	public function TurnOverAutoArray($username = null, $start = null, $end = null)
	{
		if (!empty($start) && !empty($end)) {
			if ($this->is_login) {
				$this->headers = $this->getHeaders();

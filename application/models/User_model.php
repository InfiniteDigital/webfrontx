...
class User_model extends CI_Model
{
    ...
    public function chekaccnumrow($u)
    {

        $query = $this->db->get_where('member', array('accnum' => $u));
    ...
    public function user_ufa($u)
    {
        ...
        $query = $this->db->get_where('ufa_acc', array('member_username' => $u,'type_api'=> 'ufa'));
    ...
    public function select_last_trans($u)
    {
        ...
        $sql = "SELECT * FROM `transaction` WHERE member_username = ? AND alert = 0 AND type = 1 AND stats = 0 ORDER BY id DESC LIMIT 1 ";
    ...
    public function check_ref($u,$d,$e=null)
    {
        if(empty($e)){
            $e = $d;
        }
        $sql = "SELECT * FROM `ref_log` WHERE `member_username` = ? AND `date` BETWEEN ? and ?";

        $query = $this->db->query($sql, [$u,$d,$e]);
	...
	public function getMemberIdWithUfa($u)
    {

        $sql = "SELECT * FROM `ufa_acc` WHERE `username` = ?";
	...
	public function usergen_update($id,$mem_used)
	{

		$this->db->set('used', 1);
		$this->db->set('mem_used', $mem_used);

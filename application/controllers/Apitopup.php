...
class Apitopup extends CI_Controller
{
		...
		if (isset($_POST['pro_id'])) {
			...
			if ($user) {
				...
				if ($num_deposit == 1) {
					...
					$dep_detail['amount'] = intval($dep_detail['amount']);
					...
					$credit = intval($money_ufa_split[0] . '' . $money_ufa_split[1]);

				// 	echo json_encode(['stats' => false, 'msg' => '', 'credit' => $credit, 'dep' => $dep_detail['amount'], 'check' => ($credit == $dep_detail['amount'])]);
					// die;
	...
	public function get_promotion_list()
	{
		$user = $this->session->userdata('user');
		if ($user) {
		$promotion = $this->admin_model->get_promotions1();
		echo json_encode(['status' => true, 'data' => $promotion]);
		}
	}
	public function get_promotion_by_id($id = null)
	{
		$user = $this->session->userdata('user');
		if ($user) {
		$promotion = $this->admin_model->get_promotions_byid($id);
		echo json_encode(['status' => true, 'data' => $promotion]);
		}
	}
	public function get_promotiontype_by_id($id = null)
	{
		$user = $this->session->userdata('user');
		if ($user) {
		$promotion = $this->admin_model->get_promotions_byid($id);

...

{
	...
	{
		$sql = "SELECT * FROM account_bank WHERE account_bank.type = 'scb' AND account_bank.status = 1";
	...
	public function bankall()
    {
        ...
        $sql = "SELECT * FROM `account_bank` where `status` = 1 and `type` != 'kbiz'";
        $query = $this->db->query($sql);
        $result = $query->result();
	...
	public function bankTruewallet()
	{

		$sql = "SELECT * FROM account_bank WHERE account_bank.type = 'truewallet' AND account_bank.status = 1";
		$query = $this->db->query($sql);
	...
	public function getUserAuthApiLottoHouse()
	{
		$sql = "SELECT * FROM admin WHERE mapping_auth_api = ?";
		$query = $this->db->query($sql, array('auth_lottohouse'));
		$row = $query->result_array();
		return $row;
	}

	public function getUserMapping($memberUser, $typeApi)
	{
		$sql = "SELECT * FROM ufa_acc WHERE member_username = ? AND type_api = ?";
		$query = $this->db->query($sql, array($memberUser, $typeApi));
		$row = $query->result_array();
		return $row;
	}
	...
	public function get_promotions1()
	{
		$sql = "SELECT *,promotion_type.id AS proid,promotion.id AS id FROM `promotion` LEFT JOIN promotion_type ON promotion_type.id = promotion.pro_type WHERE stats = 1";
	...
	public function get_promotions_byid($id)
	{
	...
	public function getDocs()
	{
		$this->db->select('*');
		$this->db->where('state',1);
		$this->db->from('setting_doc');
		$query = $this->db->get();
		return $query->result();
	}


	public function total_userufa()
	{
		$query = $this->db->get('ufa_acc');
